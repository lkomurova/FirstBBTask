...
export default class F10LeftColumnContainer extends LightningElement {
    ...
    showMessage = false;
    showPortal = false;
    checkFileClass = 'inactive';
    checkMessageClass = 'inactive';
    checkPortalClass = 'inactive';
    ...
    handleClick(event){
        ...
        if(event.target.name == 'files'){
            this.checkFileClass = 'active';
            this.checkMessageClass = 'inactive';
            this.checkPortalClass = 'inactive';            
                        
        }
        else if(event.target.name == 'message'){
            this.checkMessageClass = 'active';
            this.checkFileClass = 'inactive';            
            this.checkPortalClass = 'inactive';
            
            
        }
        else if(event.target.name == 'portal'){
            this.checkFileClass = 'inactive';
            this.checkMessageClass = 'inactive';            
            this.checkPortalClass = 'active';
                                 
        }
    }
    handleClickForward(event){
        if(event.target.name == 'expand'){
            this.showMenuExpanded = true;
        }
        else if(event.target.name == 'collapse'){
            this.showMenuExpanded = false;
        }
    }
    // connectedCallback(){
    //     this.handleLoad();
